...
import { ChevronLeft, ChevronRight, RotateCcw, Trash2, Pencil, Cog } from "lucide-react"
...
function ServiceCardNode({ data, selected }: NodeProps<Node<ServiceCardData>>) {
  ...
  return (
    <div className={`relative w-[420px] rounded-2xl border bg-zinc-950 text-zinc-100 ${borderColor}`}>
      <Handle
        id="in-top"
        type="target"
      ...
      <div className="flex items-center justify-between gap-2 px-3 py-2">
        <div className="flex items-center gap-2">
          <div className="h-4 w-4 rounded bg-zinc-800 border border-zinc-700 grid place-items-center">
            <span className="text-[9px]">↯</span>
          </div>
          <div className="text-sm font-semibold">{data.title}</div>
        </div>
        {data.isWorker && (
          <div className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-zinc-800 border border-zinc-700 text-[10px] text-zinc-400">
      ...
      <div className="px-3">
        <div className="rounded-lg bg-zinc-900 border border-zinc-800 p-2 text-[12px] flex items-center gap-2">
          <span className="inline-flex h-5 min-w-5 items-center justify-center rounded bg-zinc-800 text-[10px] text-zinc-300 border border-zinc-700">
            ℹ
          </span>
          <span className="opacity-80">{data.role}</span>
        </div>
      </div>

      <div className="px-3 mt-2 text-[12px] text-zinc-300">
        <div className="opacity-90">{data.description}</div>
      </div>
        ...
        <>
          ...
          <div className="px-3 py-3">
            <div className="flex items-center justify-between">
              <div className="text-[12px] text-zinc-300">Input Events</div>
            </div>
            <div className="mt-2 space-y-2">
              {inputs.map((i) => (
                <div key={i.id} className="rounded-lg border border-zinc-800 bg-zinc-900 p-2">
                  <div className="flex items-center justify-between">
                    <div className="text-[13px]">{i.name}</div>
                    <div className="flex items-center gap-2 text-zinc-300">
                      <button className="rounded p-1 hover:bg-zinc-800" aria-label="edit-input">
                        <Pencil className="h-4 w-4" />
                      </button>
                      <button className="rounded p-1 hover:bg-zinc-800" aria-label="delete-input">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                  </div>
                  {i.desc && <div className="text-[11px] opacity-70">{i.desc}</div>}
                </div>
              ))}
            </div>
          </div>
      ...
      {outputs.length > 0 && (
        <>
          ...
          <div className="px-3 py-3">
            <div className="flex items-center justify-between">
              <div className="text-[12px] text-zinc-300">Output Events</div>
            </div>
            <div className="mt-2 space-y-2">
              {outputs.map((o) => (
                <div key={o.id} className="rounded-lg border border-zinc-800 bg-zinc-900 p-2">
                  <div className="flex items-center justify-between">
                    <div className="text-[13px]">{o.name}</div>
                    <div className="flex items-center gap-2 text-zinc-300">
                      <button className="rounded p-1 hover:bg-zinc-800" aria-label="edit-output">
                        <Pencil className="h-4 w-4" />
                      </button>
                      <button className="rounded p-1 hover:bg-zinc-800" aria-label="delete-output">
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                  </div>
                  {o.desc && <div className="text-[11px] opacity-70">{o.desc}</div>}
                </div>
              ))}
            </div>
          </div>
...
const StatusEdge: React.FC<EdgeProps> = (props) => {
  ...
  return (
    <g>
      <path d={path} stroke={color} strokeWidth={2} fill="none" markerEnd={markerEnd} />
      {label && (
        <foreignObject x={labelX - 60} y={labelY - 12} width={120} height={24}>
          <div style={{ fontSize: 10, textAlign: "center", color: "#e5e7eb" }}>{label}</div>
        </foreignObject>
      )}
    </g>
  )
...
const nodeTypes = { serviceCard: ServiceCardNode }
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="inline-flex items-center gap-1 text-[10px] opacity-80">
        <span className="h-2 w-2 rounded-full bg-emerald-500 inline-block" /> ok
      </span>
      <span className="inline-flex items-center gap-1 text-[10px] opacity-80">
        <span className="h-2 w-2 rounded-full bg-amber-500 inline-block" /> running
      </span>
      <span className="inline-flex items-center gap-1 text-[10px] opacity-80">
        <span className="h-2 w-2 rounded-full bg-rose-500 inline-block" /> error
      </span>
      <span className="inline-flex items-center gap-1 text-[10px] opacity-80">
        <span className="h-2 w-2 rounded-full bg-zinc-700 inline-block" /> idle
      </span>
    </div>
  )
}
...
function WorkflowCanvas({ eventId }: { eventId?: string }) {
  ...
  return (
    <TooltipProvider>
      <div className="h-full w-full bg-zinc-950 grid grid-cols-12 rounded-lg overflow-hidden border border-zinc-800">
        {leftDrawerOpen && (
          <aside
            className={`${leftCols} border-r border-zinc-800 p-4 overflow-auto text-zinc-200 text-[12px] leading-5 transition-all duration-300`}
          >
            <div className="flex items-start justify-between mb-2">
              <div>
                <h2 className="text-lg font-semibold text-zinc-100">Species: An Onli Asset</h2>
                <div className="opacity-80">A headless marketplace for a micro‑currency</div>
                <div className="opacity-70 mb-3">Powered by Onli</div>
              </div>
              <Button
                variant="ghost"
                size="icon"
            ...
            {leftPanelTab === "overview" ? (
              ...
              <div className="space-y-4">
                <div className="text-sm font-semibold text-zinc-100 mb-3">Service Providers Summary</div>

                <div className="overflow-x-auto">
                  <table className="w-full text-[11px] border-collapse">
                    <thead>
                      <tr className="border-b border-zinc-800">
                        <th className="text-left py-2 px-2 text-zinc-400 font-medium">Service</th>
                        <th className="text-left py-2 px-2 text-zinc-400 font-medium">Purpose</th>
                        <th className="text-left py-2 px-2 text-zinc-400 font-medium">Integration</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr className="border-b border-zinc-800/50">
                        <td className="py-2 px-2 font-semibold text-zinc-100">NOWPayments</td>
                        <td className="py-2 px-2 text-zinc-300">Payment verification & crypto payouts</td>
                        <td className="py-2 px-2 text-zinc-400">Settlement Manager</td>
                      </tr>
                      <tr className="border-b border-zinc-800/50">
                        <td className="py-2 px-2 font-semibold text-zinc-100">OnliCloud</td>
                        <td className="py-2 px-2 text-zinc-300">Asset operations & ownership</td>
                        <td className="py-2 px-2 text-zinc-400">Asset Manager</td>
                      </tr>
                      <tr className="border-b border-zinc-800/50">
                        <td className="py-2 px-2 font-semibold text-zinc-100">Firefly III</td>
                        <td className="py-2 px-2 text-zinc-300">Double-entry accounting</td>
                        <td className="py-2 px-2 text-zinc-400">Settlement Manager</td>
                      </tr>
                    </tbody>
                  </table>
                ...
                <div className="space-y-6 mt-6">
                  ...
                  <div className="space-y-2">
                    <div className="text-sm font-semibold text-emerald-400">NOWPayments</div>
                    <div className="text-[11px] text-zinc-400 mb-2">
                      Payment gateway for cryptocurrency transactions
                    </div>
                    <div className="space-y-1 text-[11px]">
                      <div className="text-zinc-300 font-medium">Integration Points:</div>
                    ...
                    <div className="space-y-1 text-[11px]">
                      <div className="text-zinc-300 font-medium">API:</div>
                      <div className="bg-zinc-900 p-2 rounded font-mono text-[10px] text-zinc-400">
                        Base: https://api.nowpayments.io/v1
                      </div>
                    </div>
                    <div className="space-y-1 text-[11px]">
                      <div className="text-zinc-300 font-medium">Features:</div>
                      <ul className="list-disc list-inside space-y-1 text-zinc-400 ml-2">
                  ...
                  {/* OnliCloud */}
                  <div className="space-y-2">
                    <div className="text-sm font-semibold text-emerald-400">OnliCloud</div>
                    <div className="text-[11px] text-zinc-400 mb-2">OnliYou asset management platform (gRPC)</div>
                    <div className="space-y-1 text-[11px]">
                      <div className="text-zinc-300 font-medium">Integration Points:</div>
                      <ul className="list-disc list-inside space-y-1 text-zinc-400 ml-2">
                    ...
                    <div className="space-y-1 text-[11px]">
                      <div className="text-zinc-300 font-medium">Operations:</div>
                      <ul className="list-disc list-inside space-y-1 text-zinc-400 ml-2">
                  ...
                  {/* Firefly III */}
                  <div className="space-y-2">
                    <div className="text-sm font-semibold text-emerald-400">Firefly III</div>
                    <div className="text-[11px] text-zinc-400 mb-2">Open-source personal finance manager</div>
                    <div className="space-y-1 text-[11px]">
                      <div className="text-zinc-300 font-medium">Integration Points:</div>
                      <ul className="list-disc list-inside space-y-1 text-zinc-400 ml-2">
        ...
        <div className={`${centerCols} flex flex-col transition-all duration-300`}>
          <div className="flex items-center justify-between px-3 py-2 text-xs text-zinc-300 border-b border-zinc-800">
            <div className="flex items-center gap-2">
              ...
              <div className="tracking-wide">Onli Marketplace — VO Workflow (React Flow)</div>
            </div>
            <div className="flex items-center gap-3">
              <Button
                variant="ghost"
          ...
          <div className="flex-1 bg-zinc-950">
            ...
            >
              <Background color="#272727a" gap={16} />
              <Controls />
              <MiniMap
                nodeColor="#181818"
        ...
        {rightDrawerOpen && (
          <aside
            className={`${rightCols} border-l border-zinc-800 p-4 overflow-auto text-zinc-200 text-[12px] leading-5 transition-all duration-300`}
          >
            ...
            {selectedNode ? (
              <div className="space-y-6">
                {nodeInspectorContent[selectedNode.id] ? (
                  <>
                    <div>
                      <div className="text-[11px] uppercase tracking-wide opacity-70 mb-2">Node</div>
                      <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800">
                        <div className="font-semibold text-zinc-100">{nodeInspectorContent[selectedNode.id].node}</div>
                      </div>
                    </div>

                    <div>
                      <div className="text-[11px] uppercase tracking-wide opacity-70 mb-2">Role</div>
                      <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800">
                        <div className="flex items-start gap-2">
                          <span className="inline-flex h-5 min-w-5 items-center justify-center rounded bg-zinc-800 text-[10px] text-zinc-300 border border-zinc-700 mt-0.5">
                            ℹ
                          </span>
                          ...
                          (
                            ...
                            <div key={idx}>
                              <div className="text-emerald-400">
                                {h.key}: <span className="text-zinc-300">{h.value}</span>
                              </div>
                              {h.comment && <div className="text-zinc-500 text-[9px] ml-4">// {h.comment}</div>}
                            </div>
                          ))
                        )}
                      </div>
                    ...
                    <div>
                      <div className="text-[11px] uppercase tracking-wide opacity-70 mb-2">Logic</div>
                      <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800 space-y-2 max-h-[220px] overflow-y-auto">
                        {nodeInspectorContent[selectedNode.id].logic.map((item, idx) => (
                          <div key={idx} className="text-zinc-300 text-[11px] leading-relaxed">
                            <span className="text-cyan-400 font-medium">{idx + 1}.</span> {item}
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <pre className="p-3 bg-zinc-900 rounded-lg border border-zinc-800 font-mono text-[10px] overflow-auto max-h-[280px] text-zinc-300 whitespace-pre-wrap">
                        {codeTab === "typescript"
                  ...
                  <>
                    ...
                    <div>
                      <div className="text-[11px] uppercase tracking-wide opacity-70 mb-2">Selected Node</div>
                      <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800">
                        <div className="font-semibold text-zinc-100 mb-1">{selectedNode.data.title}</div>
                        <div className="text-zinc-400 text-[11px]">{selectedNode.data.description}</div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-zinc-300">{selectedNode.data.role}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3 max-h-[180px] overflow-y-auto">
                        <div>
                          <div className="text-[10px] text-zinc-500 mb-1">Request Headers</div>
                          <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800 font-mono text-[10px] space-y-1">
                            <div className="text-emerald-400">
                              Content-Type: <span className="text-zinc-300">application/json</span>
                            </div>
                            <div className="text-emerald-400">
                              X-Event-ID: <span className="text-zinc-300">{eventId}</span>
                            </div>
                            <div className="text-emerald-400">
                              X-Node-ID: <span className="text-zinc-300">{selectedNode.id}</span>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div>
                      <div className="text-[11px] uppercase tracking-wide opacity-70 mb-2">Logic</div>
                      <div className="p-3 bg-zinc-900 rounded-lg border border-zinc-800 space-y-3 max-h-[220px] overflow-y-auto">
                        {selectedNode.data.Input && selectedNode.data.Input.length > 0 && (
                          <div>
                            <div className="text-purple-400 font-medium text-[11px] mb-1">Input Events:</div>
                            <div className="space-y-2">
                              {selectedNode.data.Input.map((i) => (
                                <div key={i.id} className="pl-3 border-l-2 border-purple-500/30">
                                  <div className="font-mono text-[10px] text-purple-300">{i.name}</div>
                                  {i.desc && <div className="text-zinc-400 text-[10px] mt-1">{i.desc}</div>}
                                </div>
                              ))}
                            </div>
                          </div>
                        ...
                        {selectedNode.data.Output && selectedNode.data.Output.length > 0 && (
                          <div>
                            <div className="text-amber-400 font-medium text-[11px] mb-1">Output Events:</div>
                            <div className="space-y-2">
                              {selectedNode.data.Output.map((o) => (
                                <div key={o.id} className="pl-3 border-l-2 border-amber-500/30">
                                  <div className="font-mono text-[10px] text-amber-300">{o.name}</div>
                                  {o.desc && <div className="text-zinc-400 text-[10px] mt-1">{o.desc}</div>}
                                </div>
                              ))}
                            </div>
                          </div>
                    ...
                    <div>
                      ...
                      <pre className="p-3 bg-zinc-900 rounded-lg border border-zinc-800 font-mono text-[10px] overflow-auto max-h-[280px] text-zinc-300">
                        {codeTab === "typescript"
              ...
              <div className="mb-6 p-4 bg-zinc-900 rounded-lg border border-zinc-800 text-center text-zinc-500 text-[11px]">
